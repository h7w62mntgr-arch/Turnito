...
import { setBookingStatus, setDepositPaid } from "@/lib/agenda";
...
export async function cobrarSenia(_state: ActionState, formData: FormData): Promise<ActionState> {
  const owner = await requireOwner();
  const id = String(formData.get("id") ?? "");
  const paid = String(formData.get("paid") ?? "") === "true";

  try {
    if (!id) throw new SetupError("No pudimos registrar la seña.");
    await setDepositPaid(owner.businessId, id, paid);
  } catch (e) {
    return toActionError(e);
  }
  revalidatePath("/dashboard", "layout");
