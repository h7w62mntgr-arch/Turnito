...
async function load(slug: string) {
  const business = await prisma.business.findUnique({
    where: { slug },
    select: { id: true, name: true, slug: true, type: true, timezone: true, address: true },
  });
...
export default async function LigaPublicaPage({ params }: PageProps<"/liga/[slug]">) {
  const { slug } = await params;
  const data = await load(slug);
  if (!data) notFound();
  const { business, league } = data;
  ...
  if (!league) {
    return (
      <main className="mx-auto w-full max-w-2xl flex-1 px-4 py-10">
        ...
        <p className="mt-2 text-muted-foreground">La liga todavía no arrancó. ¡Pronto!</p>
      </main>
    );
  }
